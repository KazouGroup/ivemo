...
use App\Notifications\NewannoncelsNotification;
...
class NewannoncelsJob implements ShouldQueue
{
        ...
        try {
            ...
            if($this->emailsubscribannonce->count() === 0){
                ...
                Notification::route('mail',  $this->fromUser->email)
                    ->notify(new NewannoncelsNotification($this->fromUser,$this->annoncetype));

            }else{
                foreach ($this->emailsubscribannonce as $item){

                    $emailData = new NewannoncelsNotification($this->fromUser,$this->annoncetype);
