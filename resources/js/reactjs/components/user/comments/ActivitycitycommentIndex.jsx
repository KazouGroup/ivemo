...
import {loadCommentsActivitycities,likeItem,unlikeItem,unactiveItem,deleteItem} from "../../../redux/actions/commentsActions";


class ActivitycitycommentIndex extends PureComponent {
    constructor(props) {
        super(props);
    ...
    loadmoresItem() {
        this.setState((old) => {
            return { visiablecomment: old.visiablecomment + 4 }
        })
    }
    ...
    sendresponsecommentItem(e) {
        ...
        let url = route('activitycitysendresponsecomment_site',[itemCity,itemActivitycity,Id]);
    ...
    sendcommentItem(e) {
        ...
        let itemCity = this.props.match.params.city;
        let itemActivitycity = this.props.match.params.activitycity;
        let url = route('activitycitysendcomment_site',[itemCity,itemActivitycity]);
        dyaxios.post(url, item)
            .then(() => {
    ...
    updatecommentItem(e) {
        ...
        let itemActivitycity = this.props.match.params.activitycity;
        let Id = this.state.itemData.id;
        let url = route('activitycityupdatecomment_site', [itemCity,itemActivitycity,Id]);
    ...
    loadItems() {
        this.props.loadCommentsActivitycities(this.props);
      }
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            {lk.user.id !== $userIvemo.id &&(
                                                                                <button type="button" onClick={() => this.responsecommentFromItem(item)}
                                                                                        className="btn btn-default btn-neutral pull-right" title="Répondre a cette avis">
                                                                            ...
                                                                            {$userIvemo.id === item.commentable.user_id && (
                                                                                <button onClick={() => this.unactiveresponseItem(lk.id) }
                                                                                        className="btn btn-success btn-neutral pull-right" title="Masquer cette avis">
                            ...
                            {visiablecomment < comments.length && (
                                <div className="col-md-8 ml-auto mr-auto text-center">
                                    <a style={{cursor:"pointer"}} onClick={this.loadmoresItem} className="text-info">
                                        <b>Afficher la totalité des {comments.length} avis sur {this.props.name}</b>
                                    </a>
                                </div>
                            )}
...
ActivitycitycommentIndex.propTypes = {
    loadCommentsActivitycities: PropTypes.func.isRequired,
};
...
export default connect(mapStoreToProps, {
    loadCommentsActivitycities,likeItem,unlikeItem,unactiveItem,deleteItem
})(ActivitycitycommentIndex);
