...
import PrivateFavoriteBlogannonceventeList from "../../blog/blognnoncevente/inc/PrivateFavoriteBlogannonceventeList";


class PrivateUserFavoriteblogannonceventes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userfavorites:{profile:[]},
            favoritesdata:{blogannonceventes:{categoryannoncevente:[],user:[]}},
            visiable: 20,
    ...
    unfavoriteItem(id){
        Swal.fire({
            ...
            if (result.value) {
                ...
                let url = route('favoriteblogannonceventes_unfavorite.unfavorite',id);
    ...
    loadItems(){
        ...
        dyaxios.get(route('api.userdatafavoriteblogannoncevente',[itemuser])).then(response => this.setState({ favoritesdata: response.data, }));
    ...
    render() {
        ...
        const mapEmployments = favoritesdata.length >= 0 ? (
            favoritesdata.slice(0,visiable).map(item => {
                return(
                    <PrivateFavoriteBlogannonceventeList key={item.id} {...item} unfavoriteItem={this.unfavoriteItem}/>
                )
            })
        ):(
...
export default PrivateUserFavoriteblogannonceventes;
