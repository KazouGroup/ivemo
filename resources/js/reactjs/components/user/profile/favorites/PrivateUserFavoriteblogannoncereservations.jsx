...
import PrivateFavoriteBlogannoncereservationList
    from "../../blog/blogannoncereservation/inc/PrivateFavoriteBlogannoncereservationList";
import BlogannonceListSkeleton from "../../../inc/user/blog/BlogannonceListSkeleton";


class PrivateUserFavoriteblogannoncereservations extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userfavorites:{profile:[]},
            favoritesdata:{blogannoncereservation:{categoryannoncereservation:[],user:[]}},
            visiable: 20,
    ...
    unfavoriteItem(id){
        Swal.fire({
            ...
            if (result.value) {
                ...
                let url = route('favoriteblogannoncereservations_unfavorite.unfavorite',id);
    ...
    loadItems(){
        ...
        dyaxios.get(route('api.userdatafavoritfavoriteblogannoncereservation',[itemuser])).then(response => this.setState({ favoritesdata: response.data, }));
    ...
    render() {
        ...
        const mapEmployments = favoritesdata.length >= 0 ? (
            favoritesdata.slice(0,visiable).map(item => {
                return(
                    <PrivateFavoriteBlogannoncereservationList key={item.id} {...item} unfavoriteItem={this.unfavoriteItem}/>
                )
            })
        ):(
...
export default PrivateUserFavoriteblogannoncereservations;
