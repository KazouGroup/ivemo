    ...
    <div class="main-panel" id="main-panel">
        ...
        <div class="panel-header">
            <div class="header text-center">
                ...
                <p class="text-white">Toutes les conditions utilisations créé est de
                    <a href="#"><b>{{ conditionutilisations.length }}</b></a></p>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="submit text-center">
                                    <router-link :to="{ name: 'conditionutilisations.create' }" class="btn btn-round btn-primary btn-raised">
                                       <span class="btn-label">
                                        ...
                                        <b class="title_hover">New Condition utilisations</b>
                                    </router-link>
                                </div>
                            </div>
                            <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                <tr v-for="item in conditionutilisations" :key="item.id">
                                    ...
                                    <td class="text-right">
                                        ...
                                        <router-link :to="{ name: 'conditionutilisations.edit', params: { id: item.id  } }" class="btn btn-info btn-round btn-icon btn-sm edit">
...
<script>
    export default {
        data() {
            document.title = `Dashboard Policy privacy - Ivemo`;
            return {
        ...
        methods:{
            ...
            activeItem(id) {
                //Progress bar star
                this.$Progress.start();
                dyaxios.get(route('activated_conditionutilisations',id)).then(() => {
            ...
            disableItem(id) {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('unactivated_conditionutilisations',id)).then(() => {
                    /** Alert notify bootstrapp **/
                    $.notify(
                        {
            ...
            loadItems() {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('api.conditionutilisations')).then(response => {
                    this.loaded = true;
                    this.conditionutilisations = response.data;
                    this.mydatatables();
                });
                //End Progress bar
                this.$Progress.finish();
