        ...
        <div class="panel-header">
            <div class="header text-center">
                ...
                <p class="text-white">Toutes les Categories pour l'annonce location créé est de
                    <a href="#"><b>{{ categoryannoncereservations.length }}</b></a></p>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <div class="toolbar">
                                <div class="submit text-center">
                                    <button v-if="$auth.can('manage-categories')" class="btn btn-primary btn-round btn-raised " @click="newModal">
                                        ...
                                        <b class="title_hover">New Category annonces reservations</b>
                                    </button>
                                </div>
                            </div>
                            <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                <tr v-for="item in categoryannoncereservations" :key="item.id">
                                    ...
                                    <td>{{ item.annoncereservations_count }}</td>
                                    <td>{{ item.blogannoncereservations_count }}</td>
...
<script>
    ...
    export default {
        data() {
            document.title = `Dashboard Categories annonces reservations - Ivemo`;
            return {
        ...
        methods:{
            ...
            storeItem() {
                ...
                this.form.post(route('categoryannoncereservations.store')).then(() => {
            ...
            updateItem() {
                //Start Progress bar
                this.$Progress.start();
                this.form.put(route('categoryannoncereservations.update',this.form.id))
                    .then(() => {
                        //Masquer le modal après la création
                        $('#addNew').modal('hide');
                        //Event
            ...
            activeItem(id) {
                //Progress bar star
                this.$Progress.start();
                dyaxios.get(route('activated_categoryannoncereservations',id)).then(() => {
            ...
            disableItem(id) {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('unactivated_categoryannoncereservations',id)).then(() => {
                    /** Alert notify bootstrapp **/
                    $.notify(
                        {
            ...
            deleteItem(id){
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        let url = route('categoryannoncereservations.destroy',id);
            ...
            loadItems() {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('api.category_annonce_reservations')).then(response => {
                    this.loaded = true;
                    this.categoryannoncereservations = response.data;
                    this.mydatatables();
                });
                //End Progress bar
                this.$Progress.finish();
