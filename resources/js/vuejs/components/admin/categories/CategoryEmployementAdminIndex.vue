                            ...
                            <div class="card-body">
                                <div class="toolbar">
                                    <div class="submit text-center">
                                        <button v-if="$auth.can('manage-annonce-employements')" class="btn btn-primary btn-raised " @click="newModal">
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 v-show="!editmode" class="modal-title" id="addNewLabel"><b>{{this.form.name || "Add Category Emplois & Formations"}}</b></h6>
...
<script>
    ...
    export default {
        ...
        data() {
            document.title = `Dashboard Categories emplois & services ${this.user.first_name || this.name_site} - ${this.name_site}`;
            return {
                editmode: false,
                loaded: false,
                categoryemployements: {},
                form: new Form({
                    id: '',
                    name: '',
                    label: '',
        ...
        methods:{
            ...
            storeItem() {
                ...
                this.form.post(route('categoryemployements.store')).then(() => {
            ...
            updateItem() {
                //Start Progress bar
                this.$Progress.start();
                this.form.put(route('categoryemployements.update',this.form.id))
                    .then(() => {
                        //Masquer le modal après la création
                        $('#addNew').modal('hide');
                        //Event
            ...
            activeItem(id) {
                //Progress bar star
                this.$Progress.start();
                dyaxios.get(route('activated_categoryemployements',id)).then(() => {
            ...
            disableItem(id) {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('unactivated_categoryemployements',id)).then(() => {
                    /** Alert notify bootstrapp **/
                    $.notify(
                        {
            ...
            deleteItem(id){
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        let url = route('categoryemployements.destroy',id);
            ...
            loadItems() {
                //Start Progress bar
                this.$Progress.start();
                dyaxios.get(route('api.categories_categoryemployements')).then(response => {
                    this.loaded = true;
                    this.categoryemployements = response.data;
                    this.mydatatables();
                });
                //End Progress bar
                this.$Progress.finish();
